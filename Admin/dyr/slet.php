...
{
    // vi skal lige hente info
    $sql = "SELECT billede FROM dyr WHERE id = '$post[id]'";
    $query = mysqli_query($con, $sql);
    $row = mysqli_fetch_assoc($query);
    
    // først sletter vi billede filerne
    if (unlink('../../images/thumbs/'.$row['billede']) && unlink('../../images/'.$row['billede']))
    {
        // så sletter vi i databasen
        $sql = "DELETE FROM dyr WHERE id = '$post[id]'";
        $query = mysqli_query($con, $sql);
    }
    header('Location: ./');
}
...
<head>
    ...
    <title>Slet - Dyreracer - AdminZoo</title>
</head>
...
<div class="content">
    <p>
        <strong>Er du sikker på at du vil slette dyret:</strong>
    </p>
